...
SDL_Rect rectangleCoordinates = { 0, 0, 0, 0 }; //{100, 100, 200, 200};
        ...
        if (SDL_WaitEvent(&currentEvent) != 0)
        {
            ...
            if (currentEvent.type == SDL_MOUSEBUTTONDOWN)
            {
                if (currentEvent.button.button == SDL_BUTTON_LEFT)
                {
                    ...
                    mouseXClick = mouseX;
                    mouseYClick = mouseY;
                    rectangleCoordinates = { mouseXClick, mouseYClick, 0, 0 };
                }
            }
            ...
            if (currentEvent.type == SDL_MOUSEMOTION)
            {
                if (currentEvent.button.button == SDL_BUTTON_LEFT)
                {
                    ...
                    if (mouseX > mouseXClick) {
                        mouseXMove = mouseX;
                        dirX = 0;
                    }
                    else {
                        if (dirX == 0) {
                            mouseXMove = mouseXClick;
                            dirX = 1;
                        }
                        mouseXClick = mouseX;
                    }

                    if (mouseY > mouseYClick) {
                        mouseYMove = mouseY;
                        dirY = 0;
                    }
                    else {
                        if (dirY == 0) {
                            mouseYMove = mouseYClick;
                            dirY = 1;
                        }
                        mouseYClick = mouseY;
                    }

                    rectangleCoordinates = { mouseXClick, mouseYClick, mouseXMove - mouseXClick, mouseYMove - mouseYClick };
                }
