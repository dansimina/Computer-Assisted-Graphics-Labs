		...
		while (finised) {
			...
			if (simpleRejection(codeP1, codeP2)) {
				...
				if (simpleAcceptance(codeP1, codeP2)) {
					...
					if (codeP1[0] == 1 && p1.y != p2.y) { // top (intersect with yMin) 
						p1.x = p1.x + (p2.x - p1.x) * (yMin - p1.y) / (p2.y - p1.y);
						p1.y = yMin;
					}
					else {
						if (codeP1[1] == 1 && p1.y != p2.y) { // buttom (intersect with yMax)
							p1.x = p1.x + (p2.x - p1.x) * (yMax - p1.y) / (p2.y - p1.y);
							p1.y = yMax;
						}
						else {
							if (codeP1[2] == 1 && p1.x != p2.x) { // right (intersect with xMax)
								p1.y = p1.y + (p2.y - p1.y) * (xMax - p1.x) / (p2.x - p1.x);
								p1.x = xMax;
							}
							else {
								if (codeP1[3] == 1 && p1.x != p2.x) { // left (intersect with xMin)
